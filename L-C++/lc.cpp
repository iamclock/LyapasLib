...
//	strcat(strcat(strcat(strcat(strcpy(buf,"ld -o "),FileName)," "),FileName),".o");
...
//******************************************************************************** 
vector<string> Split(const char* pattern, string str)
{	vector<string> results, tmp1, tmp2, utr;
	string ex = str; //Debug(ex);
	const char *space = "^[[:space:]]*";
	while(1)
	{ tmp1 = GetStr(pattern, ex);//Debug(tmp1[0]);
	  if(tmp1[0] == "$$$") break;
	  results.push_back(tmp1[0]); 
		tmp2 = Split_Bracket("\\(", "\\)", tmp1[1]);
		utr = GetStr(space, tmp2[0]);
		if(utr[0] == "$$$")
		  results.push_back(tmp2[0]);//Debug(tmp2[0]);}
		else
		  results.push_back(utr[2]);//Debug(utr[2]);}
		results.push_back(tmp2[1]);//Debug(tmp2[1]);
		ex = tmp1[2];
	}
...
{ int perem=2, index, len;
	...
	if(index < 100 && index > 0) {
		...
		if(perem == 1) {
			...
			stext = stext + "  mov [ebp+" + NToS(SToN(addr)+400) + "],dword " + size + "\n";
...
{	string sign, code, lfunc, stext, sdata="";
	...
	while(1){ //if(name=="genecomp") Debug(code);
		...
		tmp1 = GetStr("^[[:space:]]*\\*([a-z0-9_]+)\\(([a-zZ,[:space:]LFQ0-9. ]*\\/[a-zZ,[:space:]LF0-9. ]*)\\)", code);
		...
		if(tmp1[0] != "$$$") { // Ввод/Вывод символьного комплекса
			...
			if(tmp1[4] == "<")
				stext = stext + "  dec eax\n  mov [ebp+" + NToS((SToN(tmp1[3])-1)*4 + 1020) + "],eax\n";
			continue;
		}
		...
		if(tmp1[0] != "$$$") { // Удаление/Сокращение/Обнуление комплекса
			string oper;
		...
		if(tmp1[0] != "$$$") { // Вставка/Извлечение элемента комплекса
			string adrnach, adrmoch, nomvstav;
//Debug("dir = "+tmp1[3]);
//Debug("type = "+tmp1[4]);
//Debug("nomer = "+tmp1[5]);
...
//if(name == "pramor") Debug(tmp1[1]);
			...
			if(tmp1[4][strlen(tmp1[4].c_str())-1] == 'h')	
				...
				int j=strlen(tmp1[3].c_str())-2;
				for(int i=0;i<strlen(tmp1[3].c_str())-1;i++,j--)
					if(tmp1[3][i] == '1') k|=1<<j;
				res = NToS(k);
			}
			else if(tmp1[4][0] == '0') {
...
{ vector<string> tmp, functions;
  ...
  int mstring = 1, i;
  tmp = Split("(\n|^)([a-zA-Z0-9_]+)\\(([a-zA-Z0-9,[:space:]/]+)\\)", code);// Debug(tmp[1]);
